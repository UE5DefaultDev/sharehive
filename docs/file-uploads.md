...
- **`.middleware()`**: This function runs on our server *before* an upload is permitted.
...
- **`.onUploadComplete()`**: This function runs on our server *after* the file has been successfully uploaded to storage.
